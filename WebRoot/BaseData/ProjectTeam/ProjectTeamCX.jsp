	...
	<script>
		...
		$(function(){
			...
			$('#table2').datagrid({
                ...
                nowrap: false,
                striped: true,
				url:'/jlyw/ProjectTeamServlet.do?method=2',
				//sortName: 'ProTeamCode',
				remoteSort: false,
				idField:'Id',
				frozenColumns:[[
				...
				columns:[[
					...
					{field:'ProTeamCode',title:'项目组代码',width:80,align:'center'},
					{field:'Brief',title:'拼音简码',width:80,align:'center'},					
					{field:'DeptId',title:'所属部门',width:100,align:'center',
					formatter:function(value,rowData,rowIndex){
						var datas = $('#DepartmentId').combobox('getData');
					...
					{field:'DeptCode',title:'所属部门代码',width:80,align:'center'},
				...
				toolbar:[{
					...
					handler:function(){
						...
						if(select){
							...
							$('#DepartmentId').combobox('setValue',select.DeptId);
							var Departments = $('#DepartmentId').combobox('getData');
							for(var i = 0;i<Departments.length;i++)
							{
								if(Departments[i].Id==select.DeptId)
								{
									$('#DepartmentCode').val(Departments[i].DeptCode);
									break;
								}
							}
							$('#ProjectTeamCode').val(select.ProTeamCode);
							$('#Status').combobox('setValue',select.Status);
							$('#Id').val(select.Id);
							$('#form1').form('validate');
						}else{
							$.messager.alert('warning','请选择一个项目组','warning');
							}
					}
				},'-',{
						text:'注销',
						...
						handler:function(){
							...
							if(select){
								...
								if(r){
									$.ajax({
										type:'POST',
										url:'/jlyw/ProjectTeamServlet.do?method=4',
										data:'id='+select.Id,
										dataType:"json",
				...
				onClickRow:function(rowIndex,rowData){
					...
					$('#employees').datagrid('options').queryParams={'queryProjectTeam':rowData.Id};
			...
			$('#employees').datagrid({
				...
				columns:[[
					...
					{field:'WorkLocationName',title:'所在中心',width:160,align:'center'},
		...
		function edit(){
			$('#form1').form('submit',{
				url:'/jlyw/ProjectTeamServlet.do?method=3',
		...
		$(function(){
			...
			$('#addDepartmentId').combobox({
				onSelect:function(record){
					$('#addDepartmentCode').val(record.DeptCode);
				}
			});
		});
...
<body>
<form id="frm_export" method="post" action="/jlyw/ProjectTeamServlet.do?method=7">
...
<DIV class="JlywMainLayoutDiv">
	<DIV class="JlywTopLayoutDIV">
		<jsp:include page="/Common/Title.jsp" flush="true">
			<jsp:param name="TitleName" value="项目组详细信息查询管理" />
		</jsp:include>
	</DIV>
	...
	<div region="center">
		<div>
			...
			<table id="table1">
				<tr>
               	 <td width="300" align="right">部门名称：</td>
				  <td width="150" align="left"><input id="deptname" name="deptname" type="text"></input></td>
					<td width="100" align="right">项目组名称：</td>
				  <td width="150" align="left"><input id="name" name="name" type="text"></input></td>
        ...
        <div id="p3" class="easyui-panel" style="width:800px;height:260px;padding:10px;"
				title="项目组员工" collapsible="false"  closable="false">
			<table id="employees" iconCls="icon-tip"></table>
			<br />
		</div>    
		
		<div id="add" class="easyui-window" title="新增" style="padding: 10px;width: 500px;height: 210px;overflow:hidden"
		iconCls="icon-add" closed="true" maximizable="false" minimizable="false" collapsible="false" model="true">
			...
			<table>
				...
				<tr height="30px">
					...
					<td align="left">
						<select id="addDepartmentId" name="DepartmentId" class="easyui-combobox" url="/jlyw/DepartmentServlet.do?method=6" valueField="Id" textField="Name" mode="remote" style="width:152px" required="true" panelHeight="150px"></select>
					</td>
                    <td align="right">部门代码：</td>
					<td align="left">
						<input id="addDepartmentCode" name="DepartmentCode" class="easyui-validatebox" readonly="readonly"></select>
					</td>
                </tr>
                <tr>
                	<td align="right">项目组代码：</td>
					<td align="left">
						<input id="addProjectTeamCode" name="ProjectTeamCode" class="easyui-validatebox" required="true"></select>
					</td>
				...
				<tr height="50px">
					<td align="center" colspan="2">
						<a href="#" class="easyui-linkbutton" icon="icon-add" name="Add" onclick="add()">确认提交</a>
					</td>
					<td align="center" colspan="2">
						<a href="#" class="easyui-linkbutton" icon="icon-cancel" name="cancel" href="javascript:void(0)" onclick="closed()">取消</a>
					</td>
				</tr>
			</table>
			</form>
		...
		<div id="edit" class="easyui-window" title="修改" style="padding: 10px;width: 500px;height: 210px;overflow:hidden"
		iconCls="icon-edit" closed="true" maximizable="false" minimizable="false" collapsible="false" model="true">
