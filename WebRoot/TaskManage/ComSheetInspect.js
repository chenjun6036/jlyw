	...
	$("#SearchForm").form('submit', {
		...
		success:function(data){
			...
			if(result.IsOK){
				...
				if($("#LocalMissionCode").val()!=""&&$("#LocalMissionCode").val().length>0){
					$("#AddOriginalForm-WorkDate").datebox('setValue',$("#CommissionDate").datebox('getValue'));
				}
	...
	if(row == null){
		if(type==1 || type=="1"){
			...
			if($("#SpeciesType").val()==0){
				...
				$("#function-toolbar-Model").combobox('reload','/jlyw/ApplianceServlet.do?method=2&SpeciesType=0&ApplianceSpeciesId='+$("#ApplianceSpeciesId").val());	//型号规格
				$("#function-toolbar-Range").combobox('reload','/jlyw/ApplianceServlet.do?method=3&SpeciesType=0&ApplianceSpeciesId='+$("#ApplianceSpeciesId").val());	//测量范围
...
function doOpenSelecteTemplateWindow(filesetname)	//打开选择原始记录模板文件的窗口
{
	...
	$('#template_file_grid').datagrid('options').url='/jlyw/FileDownloadServlet.do?method=5';
...
function doOpenHistoryCertificateWindow(){		//打开历史证书信息的窗口
	...
	//加载历史证书信息
	$('#history_certificate_grid').datagrid('options').url='/jlyw/OriginalRecordServlet.do?method=18';
