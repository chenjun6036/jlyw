	...
	<script>
		$(function(){			
			$('#task-table').datagrid({
                ...
                nowrap: false,
                striped: true,
				url:'/jlyw/OriginalRecordServlet.do?method=14',
...
//				sortOrder: 'desc',
	   ...
	   function doTask()	//查看明细：跳转到"检定页面ComSheetInspect.jsp"
		{
			var selectedRow = $('#task-table').datagrid('getSelected');
			if(selectedRow == null){
				$.messager.alert('提示！',"请选择一条核定任务！",'info');
				return false;
			}
		...
		function doSubmitVerify(){   	//核定通过
			$("#verify-submit-form").form('submit', {
				url:'/jlyw/OriginalRecordServlet.do?method=15',	//批量核定
				onSubmit: function(){
						// do some check
						// return false to prevent submit;
					var selectedRows = $('#task-table').datagrid('getSelections');
					if(selectedRows.length == 0){
						$.messager.alert('提示！',"请选择一个核定任务！",'info');
						return false;
					}
...
<DIV class="JlywMainLayoutDiv">
	<DIV class="JlywTopLayoutDIV">
		<jsp:include page="/Common/Title.jsp" flush="true">
			<jsp:param name="TitleName" value="检定员核定任务查询" />
		</jsp:include>
	</DIV>
	<DIV class="JlywCenterLayoutDIV">
		<table id="task-table" iconCls="icon-search"></table>

	<form method="post" action="/jlyw/TaskManage/ComSheetInspect.jsp" id="VetifyForm">
		...
		<table cellpadding="0" cellspacing="0" style="width:100%">
			<tr>
				<td style="padding-left:2px">
						<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="doTask()">查看明细</a>&nbsp;&nbsp;<a href="javascript:void(0)" class="easyui-linkbutton" title="核定通过，提交核验人进行证书核验" iconCls="icon-add" plain="true" onclick="doSubmitVerify()">核定通过</a>
					</td>
