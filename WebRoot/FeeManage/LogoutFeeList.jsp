...
//				collapsible:true,
		...
		toolbar:[{
			...
			handler:function(){
				...
				if($('#DetailListId').val() == ''){
					$.messager.alert('提示','请选择需要注销的清单','error');
				}
				else{				
					$.ajax({
						url:'/jlyw/DetailListComServlet.do?method=7',
						type:'post',
						data:'DetailListId='+$('#DetailListId').val(),
						dataType:'html',
...
function doLoadHistoryDeList()
{
	...
	$('#table4').datagrid('options').queryParams={'FeeCode':$('#FeeCode').val(),'CustomerName':encodeURI($('#CustomerName').combobox('getValue')),'Code':$('#Code').val(),'DateFrom':encodeURI($('#DateFrom').datebox('getValue')),'DateEnd':encodeURI($('#DateEnd').datebox('getValue')),'Type':"all"};
...
<DIV class="JlywMainLayoutDiv">
	<DIV class="JlywTopLayoutDIV">
		<jsp:include page="/Common/Title.jsp" flush="true">
			<jsp:param name="TitleName" value="注销费用清单" />
		</jsp:include>
	</DIV>
