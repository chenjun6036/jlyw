...
//			width:800,
			...
			onClickRow:function(rowIndex,rowData){
				if(rowData.ReportType!='校准'){
					$.messager.alert('警告','该证书不是校准证书，不可在此处修改费用!','warning');
					return;
				}
...
function commissionSheetChildFeeChange(fieldName, preValueObj, newValueObj)//委托单分费用改变时
{
	...
	if(newValue < getFieldValue(row, "OldTotalFee")){
		$.messager.alert('警告','校准证书费用不得低于标准收费！','warning');
		return false;
	}
...
function commissionSheetTotalFeeChange(preValueObj, newValueObj)//委托单总费用改变时
{
	...
	function query(){
		...
		$('#table6').datagrid('options').queryParams={'Code':encodeURI($('#Code').val()),'CustomerId':encodeURI($('#Customer').combobox('getValue')),'DateFrom':encodeURI($('#DateFrom').datebox('getValue')),'DateEnd':encodeURI($('#DateEnd').datebox('getValue'))};
