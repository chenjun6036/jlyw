...
<html>
	<head>
		...
		<%
			...
			String fileName = "";
			String doctype = "xls";
			String modFileId = "";	//模板文件ID
			String version = "-1";
			String staffId = "-1";	//检校人员（任务分配人员）
			String verifierName = "-1";	//核验人员姓名
						
			if (request.getParameter("TemplateFileId") != null
					&& !request.getParameter("TemplateFileId").equals("")) {
				modFileId = request.getParameter("TemplateFileId");
			...
			if(request.getParameter("StaffId") != null 
					&& !request.getParameter("StaffId").equals("")){
...
function WebOffice1_NotifyCtrlReady() {
	...
	<%	if(modFileId.equals("")) {	%>		// 装载已存在的文档
			document.all.WebOffice1.LoadOriginalFile("../FileDownloadServlet.do?method=2&OriginalRecordId=<%=oRecordId %>", "<%=doctype%>");
	
	<%	} else{		%>	//装载模板并填入数据
			document.all.WebOffice1.LoadOriginalFile("../FileDownloadServlet.do?method=3&OriginalRecordId=<%=oRecordId %>&TemplateFileId=<%=modFileId %>&DownloadType=1&VerifierName="+encodeURI('<%=verifierName %>'), "<%=doctype%>");
						...
						<tr>
							...
							<td align="right">
								<input name="button" type=button
									...
									value="  返回  ">&nbsp;
								<input name="button" type=button
									onClick="return TempSaveOriginalRecord('<%=fileName%>','<%=oRecordId%>','<%=version%>')"
									value="  暂存  ">&nbsp;
								...
								<input name="button93" type="button"
									onClick="return SubmitOriginalRecord('<%=fileName%>','<%=oRecordId%>','<%=version%>')" value="  提交  "
									classs="rollout">
								<% }%>
							</td>
						</tr>
