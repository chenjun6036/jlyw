...
function SaveDoc(id,docType) {
	try{
		...
		alert("begin save");
		returnValue = webObj.HttpPost("../FileUploadServlet.do?method=2");	// 判断上传是否成功
		var retVal = eval("("+returnValue+")");
		if(true == retVal.IsOK){
			alert("文件上传成功");	
		}else{
			alert(retVal.msg);
...
*
/****************************************************/
function TempSaveOriginalRecord(fileName, oRecordId, version) {
	try{
		var webObj=document.getElementById("WebOffice1");
		var returnValue;
		...
		webObj.HttpAddPostCurrFile("DocContent","");		// 上传文件
		returnValue = webObj.HttpPost("../FileUploadServlet.do?method=3");	// 判断上传是否成功
		var retVal = eval("("+returnValue+")");
		if(true == retVal.IsOK){
			alert("原始记录暂存成功！");	
			self.close(); 
...
*
/****************************************************/
function SubmitOriginalRecord(fileName, oRecordId, version) {
	try{
		var webObj=document.getElementById("WebOffice1");
		var returnValue;
		 if(fileName == null || fileName == ""){
		...
		var retVal = eval("("+returnValue+")");
		if(true == retVal.IsOK){
			if(retVal.msg!=null&&retVal.msg.length>0){
				alert(retVal.msg);
			}else{
				alert("原始记录提交成功！");	
			}			
...
*
/****************************************************/
function TempSaveCertificate(fileName, oRecordId, version) {
	try{
		var webObj=document.getElementById("WebOffice1");
		var returnValue;
		 if(fileName == null || fileName == ""){
		...
		webObj.HttpAddPostString("Version", version);
		webObj.HttpAddPostCurrFile("DocContent","");		// 上传文件
		returnValue = webObj.HttpPost("../FileUploadServlet.do?method=7");	// 判断上传是否成功
		var retVal = eval("("+returnValue+")");
		if(true == retVal.IsOK){
			alert("证书暂存成功！");	
			self.close(); 
...
*
/****************************************************/
function SubmitEditCertificate(fileName, oRecordId, version,flag) {
	try{
		var webObj=document.getElementById("WebOffice1");
		var returnValue;
		 if(fileName == null || fileName == ""){
		...
		if(flag==1||flag=='1'){
			returnValue = webObj.HttpPost("../FileUploadServlet.do?method=12");	// 判断上传是否成功
		}else{
