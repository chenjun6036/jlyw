 ...
 * Reason entities. Transaction control of the save(), update() and delete()
 * operations can directly support Spring container-managed transactions or they
 * can be augmented to handle user-managed Spring transactions. Each of these
 * methods provides additional information for how to configure it for the
 * desired type of transaction control.
 * 
 * @see com.jlyw.hibernate.Reason
 * @author MyEclipse Persistence Tools
 */

public class ReasonDAO extends BaseHibernateDAO {
	private static final Logger log = LoggerFactory.getLogger(ReasonDAO.class);
	// property constants
	...
	public List findByExample(Reason instance) {
		...
		try {
			List results = getSession().createCriteria(
					"com.jlyw.hibernate.Reason").add(Example.create(instance))
					.list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Reason instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Reason as model where model."
					+ propertyName + "= ?";
	...
	public List findAll() {
		...
		try {
			String queryString = "from Reason";
			Query queryObject = getSession().createQuery(queryString);
			return queryObject.list();
		} catch (RuntimeException re) {
